...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setSuccess("Пароль обновлён. Переходим в кабинет…");
    try {
      const res = await fetch("/api/me");
      const json = await res.json();
      const route =
        json.role === "operator" || json.role === "admin"
          ? "/operator/dashboard"
          : "/user/orders";
      window.setTimeout(() => {
      ...
      {linkInvalid || !hasSession ? (
        <div className="mt-5 space-y-4">
          <div className="rounded-2xl bg-red-50 px-4 py-3 text-center text-xs font-bold text-red-500">
            Ссылка недействительна или устарела. Запросите новую из формы входа.
          </div>
          <a
            href="/?auth=required"
        ...
        <form onSubmit={handleSubmit} className="mt-5 space-y-5">
          ...
          {success ? (
            <div className="rounded-2xl bg-green-50 px-4 py-3 text-center text-xs font-bold text-green-600">
              {success}
            </div>
          ) : null}
