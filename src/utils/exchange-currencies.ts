...
import { SBP_BANKS, type SbpBank } from "@/src/utils/banks/sbp-banks";
...
function sbpBankToFiatCurrency(bank: SbpBank): ExchangeCurrency {
    return {
        id: `rub_${bank.id}`,
        name: `${bank.name} RUB`,
        code: "RUB",
        orderCode: `RUB_${bank.id.toUpperCase()}`,
        iconSrc: bank.iconSrc,
        network: {
            id: bank.id,
            label: bank.name,
            shortLabel: bank.name,
            description: `Перевод по СБП или на карту ${bank.name}.`,
            addressHint: "+7 … или номер карты",
        },
    };
}
...
export const FIAT_CURRENCIES: ExchangeCurrency[] = [
    {
        ...
        network: {
            ...
            shortLabel: "СБП",
            description: "Выплата через СБП на телефон или карту получателя.",
            addressHint: "+7 … или номер карты",
        },
    },
    ...SBP_BANKS.map(sbpBankToFiatCurrency),
];
        ...
        networks: [
            ...
            },
            {
                ...
                network: {
                    ...
                    shortLabel: "SOL",
                    description:
                        "USDT в сети Solana (SOL). Не используйте TRON, Ethereum, BSC или TON.",
        ...
        networks: [
            {
                ...
                network: {
                    ...
                    label: "Solana",
                    shortLabel: "SOL",
                    description: "Перевод в сети Solana (SOL).",
...
export const CRYPTO_CURRENCIES: ExchangeCurrency[] = CRYPTO_ASSETS.flatMap(
    (asset) =>
        asset.networks.map((variant) => variantToCurrency(asset, variant)),
);
...
/** Банк из валюты: sber для rub_sber, пусто для общего СБП. */
export function fiatBankId(currency: ExchangeCurrency): string {
    if (currency.id === "sbp" || currency.id === "rub_cash") return "";
    const id = currency.network?.id || currency.id.replace(/^rub_/, "");
    return SBP_BANKS.some((bank) => bank.id === id) ? id : "";
}

export function isBankFiatCurrency(currency: ExchangeCurrency): boolean {
    return currency.id === "sbp" || Boolean(fiatBankId(currency));
}

export function isBankFiatCurrencyId(currencyId: string): boolean {
    if (currencyId === "sbp") return true;
    if (currencyId === "rub_cash") return false;
    return SBP_BANKS.some((bank) => currencyId === `rub_${bank.id}`);
}
